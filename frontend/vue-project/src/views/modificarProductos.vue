...
const guardarProducto = async () => {
 ...
 if (producto.value.imagen) {
  ...
  producto.value.imagen = url_imagen
 }
 if((imagePreview.value !== null) && !(producto.value.imagen)){
  producto.value.imagen=imagePreview.value
 }
...
const cargar = async () => {
  try {
    ...
    const dataProducto = await axios.get(`http://localhost:8000/api/products/${route.params.id}`)
    const dataStock = await axios.get(`http://localhost:8003/api/${route.params.id}`)
    producto.value={
      category_id:1489,
      descripcion:dataProducto.data.description,
      image_url:dataProducto.data.image_url,
      nombre:dataProducto.data.name,
      precio:dataProducto.data.price,
      product_id:dataProducto.data.product_id,
      cantidad:dataStock.data.cantidad
    }
    imagePreview.value =dataProducto.data.image_url
  } catch (error) {
